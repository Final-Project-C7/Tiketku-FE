...
const DestinasiFav = () => {
  ...
  return (
    <div className="destinasi-fav position-relative d-flex flex-column mx-auto">
      ...
      <input className="col-12 col-sm-11 search-desti ms-3 mt-3 mb-2" type="search" placeholder="   Cari ..." aria-label="Search" onChange={(e) => setSearchQuery(e.target.value)} />
      ...
      <Container>
        <div className="destinasi-card d-flex justify-content-evenly flex-wrap mb-5 mx-auto">
          {data?.data
            ?.filter((flight) => [3, 4, 6, 7, 9, 8, 12, 14, 15, 16, 17, 19].includes(flight.id))
            .filter((flight) => {
              if (selectedCategory === "") {
                return true;
              }
            ...
            .filter((flight) => {
              ...
              const departureCity = flight?.departureAirport?.city.toLowerCase();
              return departureCity.includes(searchQuery.toLowerCase());
            })
            .map((flight) => (
              <Card className="destinasi-card__list shadow mt-3" style={{ cursor: "pointer" }} key={flight.id}>
                <Link to={`/checkout`} state={flight}>
                  ...
                  <Card.Body className="pt-2">
                    ...
                    <Card.Text className="destinasi-card__text-1 mb-1 text-black">
                      Mulai dari
                      <span className="text-danger fw-bold"> IDR {flight.economyClass_price}</span>
                    </Card.Text>
                  </Card.Body>
                </Link>
              </Card>
