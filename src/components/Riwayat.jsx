  ...
  useEffect(() => {
    const fetchBookingData = async () => {
      try {
        ...
        console.log(user_id);
        const response = await axios.get(`https://c7-tiketku.up.railway.app/api/v1/bookings?user_id=${user_id}`, { headers });
                  ...
                  bookingData.map((booking) => (
                    <div key={booking.id} onClick={() => handleCardClick(booking.id)}>
                      <div className="mb-3">
                        ...
                        <div className="checkout-biodata-card border border-3 p-3 rounded-3 mb-2">
                          ...
                          <div className="d-flex">
                            ...
                            <div className="col-3">
                              <p className="fw-bold mb-0 text-dark">{booking?.flight?.departureAirport?.city}</p>
                              <p className="mb-0">{Moment(booking?.flight?.departure_time).format("dddd, Do MMMM  YYYY")}</p>
                              <p className="mb-0"></p>
                            </div>
                            ...
                            <div className="col-3 d-flex flex-column">
                              <p className="fw-bold mb-0 text-dark">{booking?.flight?.arrivalAirport?.city}</p>
                              <p className="mb-0">{Moment(booking?.flight?.arrival_time).format("dddd, Do MMMM  YYYY")}</p>
                              <p className="mb-0"></p>
                            </div>
                          </div>
                          ...
                          <div className="d-flex align-items-center">
                            <div className="col-4">
                              ...
                              <p className="mb-0">{booking?.payment?.payment_code}</p>
                            </div>
