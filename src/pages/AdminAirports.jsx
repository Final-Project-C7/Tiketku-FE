...
import FormModalUpdateAdminAirports from "../components/Form/FormModalUpdateAdminAirports";
...
function AdminAirports() {
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`https://c7-tiketku.up.railway.app/api/v1/airports/${deleteAirportId}`);
      setIsDeleted(true);
                    ...
                    <tbody>
                      {data?.data?.airport.map((airport, index) => (
                        <tr key={airport.id}>
                          ...
                          <td>
                            <div className="d-flex">
                              <FormModalUpdateAdminAirports data={airport} />
